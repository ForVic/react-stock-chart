...
function Stock() {
  ...
  const [prevPrice, setPrevPrice] = useState(-2);
  const [priceTime, setPriceTime] = useState(null);

  async function getStockPrice(state) {
    const proxyURL = "https://cors-anywhere.herokuapp.com/";
    let stocksURL =
      proxyURL + "https://query1.finance.yahoo.com/v8/finance/chart/" + state;
  ...
  useEffect(() => {
    let timeoutId;
    async function getLatestPrice(state) {
      try {
        const data = await getStockPrice(state);
  ...
  return (
    <div className="stock-info">
      <div className="ticker">{state}</div>
